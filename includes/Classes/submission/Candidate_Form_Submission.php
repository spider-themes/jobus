...
class Candidate_Form_Submission {
	...
	public static function get_candidate_specifications( int $candidate_id ): array {
		...
		$specifications = array(
			'specifications' => isset( $meta['candidate_specifications'] ) && is_array( $meta['candidate_specifications'] ) ? $meta['candidate_specifications']
				: array(),
	...
	public function save_candidate_location( int $candidate_id, array $post_data ): bool {
		...
		$meta['candidate_location_address'] = isset( $post_data['candidate_location_address'] )
			? sanitize_text_field( $post_data['candidate_location_address'] ) : '';
	...
	private function save_candidate_cv( int $candidate_id, array $post_data ): void {
		...
		if ( $action === 'delete' ) {
			...
			if ( ! empty( $post_data['cv_attachment_id'] ) ) {
				...
				if ( isset( $file_data ) && is_array( $file_data ) && isset( $file_data['name'] ) && isset( $file_data['type'] )
		...
		if ( isset( $post_data['education'] ) && is_array( $post_data['education'] ) ) {
			foreach ( $post_data['education'] as $key => $edu ) {
				...
				if (
					...
					|| ( empty( $edu['academy'] ) && empty( $edu['title'] ) && empty( $edu['description'] )
					     && empty( $edu['sl_num'] ) ) // Check if all fields are empty
				) {
					continue;
				}
	...
	private function handle_form_submission(): void {
		...
		if ( isset( $post_data['candidate_location_address'] ) || isset( $post_data['candidate_location_lat'] )
		     || isset( $post_data['candidate_location_lng'] )
		) {
