...
class Nav_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ): void {
		...
		$regular_img  = get_post_meta( $data_object->ID, 'jobus_menu_image', true );
		...
		if ( !empty($regular_img) && !empty($regular_img['url']) ) {
			$item_output .= '<img src="'.esc_url($regular_img['url']).'" data-src="' . esc_url($regular_img['url']) . '" alt="" class="lazy-img">';
		}
