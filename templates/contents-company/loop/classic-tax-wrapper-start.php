 ...
 *
 * This template can be overridden by copying it to yourtheme/jobus/contents-company/loop/classic-tax-wrapper-start.php.
 *
...
if ( isset( $taxonomy ) && ! empty( $taxonomy ) ) {
	$company_taxonomy = ! empty( $_GET[ $taxonomy ] ) ? array_map( 'sanitize_text_field', wp_unslash( $_GET[ $taxonomy ] ) ) : [];
}

if ( ! empty( $company_taxonomy ) ) {
...
if ( $taxonomy === 'jobus_company_location' ) {
...
<div class="filter-block bottom-line pb-25 mt-25">
    <a class="filter-title fw-500 text-dark<?php echo esc_attr( $is_collapsed ) ?>" data-bs-toggle="collapse"
       href="#collapse-<?php echo esc_attr( $taxonomy ) ?>" role="button"
